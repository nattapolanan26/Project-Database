                ...
                <div class="card">
                    <div class="card-body">
						<form action="#" id="myformshow" name="myformshow" method="get">
							...
							$sql = "SELECT product_name,categorycolor.*,costprice.*,company.cpn_name,brand.brand_name,color.color_name,cc_volume
                    FROM product
                    ...
                    WHERE categorycolor.product_id = '$idproduct'
                        ...
                        <th>จัดการ</th>
							...
							do {
								if ($row['product_id'] != '') {
									...
									echo "<td>" . $row["product_name"] . " " . $row['brand_name'] . " " . "สี" . $row['color_name'] . " " . $row['cc_volume'] . "</td> ";
									echo "<td>" . $row["cpn_name"] . "</td> ";
									echo "<td align='right' width='10%'>" . number_format($row["costprice"], 2) . "</td> ";
									//แก้ไข
									echo "<td align='center' width='15%'><a href='cgrcolor_costprice_edit.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]' class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i>&ensp;แก้ไข</a>
							<a href='#' onclick=\"JavaScript:confirmdelete('cgrcolor_costprice_delete.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]')\" class='btn btn-danger btn-xs'><i class='fa fa-trash'></i>&ensp;ลบ</a></td> ";
									echo "</tr>";
								}
