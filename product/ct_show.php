                ...
                <div class="card">
                    <div class="card-body">
						<form action="#" id="myformshow" name="myformshow" method="post">
							<h4>เครื่องมือช่าง</h4>
							<div class="form-inline">
								<!-- เพิ่มขอมูล -->
								<div class="form-group mb-2">
									<a href="product.php?act=add" class="btn-danger btn-sm"><i class='fa fa-undo' aria-hidden='true'></i> ย้อนกลับ</a>&ensp;
									<a href="ct_insert.php" class="btn-info btn-sm"><i class='fa fa-plus' aria-hidden='true'></i> เพิ่มเครื่องมือช่าง</a>
								</div>
							...
							$sql = "SELECT product.product_id,product.product_name,product.product_saleprice,brand_name,color_name,unit_name,craftmantool.ct_size,material.*
					FROM product 
					LEFT JOIN craftmantool ON product.product_id = craftmantool.product_id
					...
					ORDER BY craftmantool.product_id ASC";
						...
						<th>จัดการ</th>
							...
							do {
								if ($row['product_id'] != '') {
									...
									echo "<td align='center'>" . $row["ct_size"] . "</td> ";
									echo "<td align='right'>" . number_format($row["product_saleprice"], 2) . "</td> ";
									//เพิ่มราคาทุนสินค้า
									echo "<td align='center' width='10%'><a href='ct_costprice.php?p_id=$row[product_id]'><img src='/picture/add.png' width='30' height='30' alt='เพิ่มราคาสินค้า'></a>&emsp;<a href='ct_costprice_show.php?p_id=$row[product_id]'><img src='/picture/product.png' width='30' height='30' alt='ดูสินค้า'></a></td> ";
									//แก้ไข
									echo "<td align='center' width='15%'><a href='ct_edit.php?p_id=$row[product_id]' class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i>&ensp;แก้ไข</a>
							<a href='#' onclick=\"JavaScript:confirmdelete('ct_delete.php?p_id=$row[product_id]')\" class='btn btn-danger btn-xs'</a><i class='fa fa-trash'></i>&ensp;ลบ</td> ";
									echo "</tr>";
								}
