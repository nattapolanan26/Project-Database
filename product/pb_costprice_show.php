				...
				<div class="card">
					<div class="card-body">
						<form action="#" id="myformshow" name="myformshow" method="get">
							...
							$sql = "SELECT product.*,plumbling.*,brand.brand_name,unit.unit_name,class,company.*,costprice
                    FROM product
                    ...
                    WHERE costprice.product_id = '$idproduct'";
						...
						<th>จัดการ</th>
							...
							do {
								if ($row['product_id'] != '') {
									...
									echo "<td>" . htmlspecialchars($row["product_name"] . " " . $row["brand_name"] . " " . $row["class"] . " " . $row['pb_thick'] . " " . "ขนาด" . " (" . $row['pb_size'] . ")") . "</td> ";
									echo "<td>" . $row["cpn_name"] . "</td> ";
									echo "<td align='right' width='10%'>" . number_format($row["costprice"], 2) . "</td> ";
									//แก้ไข
									echo "<td align='center' width='15%'><a href='pb_costprice_edit.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]' class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i>&ensp;แก้ไข</a>
							<button type='button' onclick=\"JavaScript:confirmalert('pb_costprice_delete.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]')\" class='btn btn-danger btn-xs'><i class='fa fa-trash'></i>&ensp;ลบ</button>
							</td> ";
									echo "</tr>";
								}
