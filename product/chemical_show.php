						...
						<form action="#" id="myformshow" name="myformshow" method="post">
							...
							<div class="form-inline">
								...
								<div class="form-group mb-2">
									...
									<a href="chemical_insert.php" class="btn-info btn-sm"><i class='fa fa-plus' aria-hidden='true'></i> เพิ่มเคมีภัณฑ์</a>
								</div>
							...
							$sql = "SELECT product.product_id,product.product_name,product.product_saleprice,brand_name,unit_name,chemicalsolution.cs_volume,material.*
					FROM product 
					INNER JOIN chemicalsolution ON product.product_id = chemicalsolution.product_id
						...
						<tr bgcolor='#AED6F1' align='center' style='font-weight:bold'>
							...
							do {
								if ($row['product_id'] != '') {
									...
									echo "<td align='right'>" . $row["cs_volume"] . "</td> ";
									echo "<td align='right'>" . number_format($row["product_saleprice"], 2) . "</td> ";
									//เพิ่มราคาทุนสินค้า
									echo "<td align='center'><a href='chemical_costprice.php?p_id=$row[product_id]'><img src='/picture/add.png' width='30' height='30' alt='เพิ่มราคาสินค้า'></a>&emsp;<a href='chemical_costprice_show.php?p_id=$row[product_id]'><img src='/picture/product.png' width='30' height='30' alt='ดูสินค้า'></a></td> ";

									//แก้ไข
									echo "<td align='center'><a href='chemical_edit.php?p_id=$row[product_id]' class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i>&ensp;แก้ไข</a>
							<a href='#' onclick=\"JavaScript:confirmdelete('chemical_delete.php?p_id=$row[product_id]')\" class='btn btn-danger btn-xs'</a><i class='fa fa-trash'></i>&ensp;ลบ</td> ";
									echo "</tr>";
								}
