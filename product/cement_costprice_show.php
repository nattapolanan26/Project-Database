					...
					<div class="card-body">
						<form action="#" id="myformshow" name="myformshow" method="get">
							...
							<div class="form-inline mb-2">
								<a href="cement_show.php" class="btn-danger btn-sm"><i class='fa fa-undo' aria-hidden='true'></i> ย้อนกลับ</a>
							</div>
							<?php
							$search = isset($_GET['search']) ? $_GET['search'] : '';
							$idproduct = isset($_GET['p_id']) ? $_GET['p_id'] : '';

							$sql = "SELECT product.*,cement.*,costprice.*,company.cpn_name,brand.brand_name,color.color_name,company.cpn_id
                    FROM product
					INNER JOIN cement ON cement.product_id = product.product_id
						...
						<th>จัดการ</th>
							...
							do {
								if ($row['product_id'] != '') {
									...
									echo "<td>" . $row["product_name"] . " " . $row['brand_name'] . " " . "สี" . $row['color_name'] . " " . $row['cm_volume'] . "</td> ";
									echo "<td>" . $row["cpn_name"] . "</td> ";
									echo "<td align='right' width='10%'>" . number_format($row["costprice"], 2) . "</td> ";
									//แก้ไข
									echo "<td align='center' width='15%'><a href='cement_costprice_edit.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]' class='btn btn-warning btn-xs'><i class='fa fa-pencil'></i>&ensp;แก้ไข</a>
							<button type='button' onclick=\"JavaScript:confirmalert('cement_costprice_delete.php?p_id=$row[product_id]&cpn_id=$row[cpn_id]')\" class='btn btn-danger btn-xs'><i class='fa fa-trash'></i>&ensp;ลบ</button></td> ";
									echo "</tr>";
								}
