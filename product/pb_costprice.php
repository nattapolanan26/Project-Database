...
<?php include('../connectdb.php'); ?>
            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="pb_show.php" class="btn-danger btn-sm"><i class='fa fa-undo' aria-hidden='true'></i> ย้อนกลับ</a>
                        <form action="#" name="addcostprice" id="addcostprice" method="post" enctype="multipart/form-data">
                            ...
                            $pvcid = isset($_GET['pvc_id']) ? $_GET['pvc_id'] : '';

                            $sql = "SELECT product.*,plumbling.*,brand.brand_name,unit.unit_name,class
                    FROM product
                    ...
                    WHERE product.product_id = '$productid'";
                            ...
                            <div class="form-row">
                                <div class="col-md-5 mb-3">
                                    ...
                                    <input class="form-control" type="text" value="<?= htmlspecialchars($productname . " " . $brandname . " " . $class . " " . $thick . " " . "ขนาด" . " (" . $size . ") " . "ราคา" . " " . number_format($saleprice, 2)); ?>" disabled>
                                </div>
                            </div>
    ...
    if (isset($_POST['addcostprice'])) {
        ...
        $Sqlsaleprice = "SELECT product_saleprice FROM product INNER JOIN plumbling ON plumbling.product_id = product.product_id WHERE plumbling.product_id = '$idproduct'";
        ...
        if ($_POST['company'] && $_POST['costprice'] != '') {
            if ($_POST['costprice'] <  0) {
                ...
                echo "window.location.href='pvc_costprice.php?p_id=$productid'"; //แสดงหน้าก่อนนี้
                echo "</script>";
                ...
                if ($conn->query($sql) == true) {
                    echo "<script>"; //คำสั่งสคิป
                    echo "window.location.href='pb_show.php';"; //แสดงหน้าก่อนนี้
                    echo "</script>";
                } else {
                    echo "<script>"; //คำสั่งสคิป
                    echo "alert('This list has been added to the database already.');"; //แสดงหน้าต่างเตือน
                    echo "window.location.href='pb_costprice.php?p_id=$productid';"; //แสดงหน้าก่อนนี้
                    echo "</script>";
                }
            } else {
                echo "<script>"; //คำสั่งสคิป
