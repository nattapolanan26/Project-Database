...
<body>
            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="cgr_color_show.php" class="btn-danger btn-sm">ย้อนกลับ</a>
                        <form action="#" name="addcostprice" id="addcostprice" method="post" enctype="multipart/form-data">
                            ...
                            $sql = "SELECT product.*,categorycolor.*,brand.brand_name,unit.unit_name,color.color_name
                    FROM product 
                    INNER JOIN categorycolor ON product.product_id = categorycolor.product_id
                    INNER JOIN unit ON unit.unit_id = product.unit_id 
                    INNER JOIN brand ON brand.brand_id = product.brand_id
                    INNER JOIN color ON color.color_id = categorycolor.color_id
                    WHERE categorycolor.product_id = '$productid'";
                            $result = mysqli_query($conn, $sql)  or die("Error description: " . mysqli_error($conn));
                            while ($row = mysqli_fetch_array($result)) {
                                $productname = $row['product_name'];
                                $brandname = $row['brand_name'];
                                $colorname = $row['color_name'];
                                $volume = $row['cc_volume'];
                            ...
                            <div class="form-row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input class="form-control" type="text" value="<?php echo $productname . " " . $brandname . " " . "สี" . $colorname . " " . $volume . " " . "ราคา" . " " . number_format($saleprice, 2); ?>" disabled>
                                </div>
                            </div>
    ...
    if (isset($_POST['addcostprice'])) {
        ...
        $Sqlsaleprice = "SELECT product.product_saleprice FROM product INNER JOIN categorycolor ON product.product_id = categorycolor.product_id WHERE product.product_id = '$idproduct'";
        ...
        if ($_POST['company'] && $_POST['costprice'] != '') {
            if ($_POST['costprice'] <  0) {
                ...
                echo "window.location.href='cgrcolor_costprice.php?p_id=$productid'"; //แสดงหน้าก่อนนี้
                echo "</script>";
